...
public class FormattableURL implements Formattable {
    ...
    public FormattableURL(URL delegate) {
    ...
    public void formatTo(Formatter formatter, int flags, int width, int precision) {
        ...
        if (upperCase && (scheme != null)) {
            scheme.toUpperCase(Locale.ENGLISH);
        }
        String hostname = delegate.getHost();
        if(upperCase && (hostname != null)) {
            hostname.toUpperCase(Locale.ENGLISH);
        }
        ...
        boolean leftJustify = (FormattableFlags.LEFT_JUSTIFY & flags) != 0;
